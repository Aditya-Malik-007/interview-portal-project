...
function Dashboard({ user }) {
  ...
  const fetchMyPosts = async () => {
    try {
      const res = await axios.get('https://interview-portal-project-1.onrender.com/api/my-posts', { withCredentials: true });
      setPosts(res.data);
    } catch (err) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.put(
        `https://interview-portal-project-1.onrender.com/api/experiences/${editingPost}`,
        editForm,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://interview-portal-project-1.onrender.com/api/experiences/${id}`, { withCredentials: true });
      setIsError(false);
